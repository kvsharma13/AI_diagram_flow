...
import { ListChecks, Plus, Trash2, ArrowUp, ArrowDown, ChevronsUpDown } from 'lucide-react';
...
const STATUS_COLOR: Record<RequirementStatus, string> = {
  Draft: '#A1A1AA', Approved: '#22C55E', Rejected: '#EF4444',
};
const PRIORITY_COLOR: Record<MoSCoW, string> = {
  'Must Have': '#EF4444', 'Should Have': '#F59E0B', 'Could Have': '#38BDF8', "Won't Have": '#71717A',
};
const PRIORITY_RANK: Record<MoSCoW, number> = {
  'Must Have': 0, 'Should Have': 1, 'Could Have': 2, "Won't Have": 3,
};

type SortKey = 'reqId' | 'description' | 'type' | 'priority' | 'status' | 'source';

const cellInput =
  'w-full bg-transparent px-2 py-1.5 text-sm rounded focus:outline-none focus:ring-2 focus:ring-[var(--accent)]';
...
export default function RequirementsEditor() {
  ...
  const add = () => {
    ...
    setRequirements([
      ...requirements,
      { id: uuidv4(), reqId, description: '', type: 'Functional', priority: 'Should Have', status: 'Draft', source: '' },
    ]);
  };
  ...
  const counts = {
    ...
    must: requirements.filter((r) => r.priority === 'Must Have').length,
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-2 mb-4">
        ...
        <div className="w-40">
          <Select value={fPriority} onChange={(e) => setFPriority(e.target.value as any)}>
        ...
        {(fType !== 'All' || fPriority !== 'All' || fStatus !== 'All') && (
          <button
            onClick={() => { setFType('All'); setFPriority('All'); setFStatus('All'); }}
      ...
      {requirements.length === 0 ? (
        <div className="rounded-xl border-2 border-dashed p-14 text-center" style={{ background: 'var(--surface-1)', borderColor: 'var(--border)' }}>
          ...
          <p className="text-sm mb-6 max-w-md mx-auto" style={{ color: 'var(--text-secondary)' }}>
            Add requirements manually, or generate a first draft from your project brief with AI.
          </p>
            ...
            <table className="w-full" style={{ minWidth: 980 }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--border)' }} className="text-xs">
                  ...
                  <Th k="type" label="Type" className="w-44" />
                  <Th k="priority" label="Priority" className="w-44" />
                  <Th k="status" label="Status" className="w-40" />
                  <Th k="source" label="Source" className="w-40" />
                  <th className="w-10" />
                </tr>
              </thead>
              <tbody>
                {visible.map((r, idx) => (
                  <tr
                    key={r.id}
                    className="group"
                    style={{ borderBottom: idx === visible.length - 1 ? 'none' : '1px solid var(--divider)' }}
                  >
                    <td className="px-2 py-1.5 text-xs font-mono whitespace-nowrap" style={{ color: 'var(--text-muted)' }}>{r.reqId}</td>
                    ...
                    <td className="px-1 py-1">
                      <Select value={r.type} onChange={(e) => update(r.id, { type: e.target.value as RequirementType })}>
                        {TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
                      </Select>
                    </td>
                    <td className="px-1 py-1">
                      <Select
                        value={r.priority}
                        style={{ color: PRIORITY_COLOR[r.priority], fontWeight: 500 }}
                        onChange={(e) => update(r.id, { priority: e.target.value as MoSCoW })}
                      >
                    ...
                    <td className="px-1 py-1">
                      <Select
                        value={r.status}
                        style={{ color: STATUS_COLOR[r.status], fontWeight: 500 }}
              ...
              <div key={i} className="rounded-lg p-3" style={{ background: 'var(--surface-2)', border: '1px solid var(--border)' }}>
                <div className="flex items-center gap-2 mb-1">
                  <span
                    ...
                    style={{
                      background: it.type === 'Non-Functional' ? 'rgba(167,139,250,0.15)' : 'rgba(37,99,235,0.15)',
                      color: it.type === 'Non-Functional' ? '#A78BFA' : 'var(--accent-hover)',
                    }}
                  >
                    {it.type || 'Functional'}
                  </span>
                  <span className="text-[10px] font-medium" style={{ color: PRIORITY_COLOR[(it.priority as MoSCoW)] || 'var(--text-muted)' }}>
                    {it.priority || 'Should Have'}
                  </span>
                </div>
